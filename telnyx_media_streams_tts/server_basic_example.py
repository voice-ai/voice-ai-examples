...
# Supported paths in this baseline:
#   ulaw_8000 -> ulaw_8000
#   pcm_16000 -> l16_16000
TELNYX_AUDIO_FORMAT = "l16_16000"
...
TTS_TEXTS = [
    "This is utterance one of the Voice AI websocket relay test.",
    "This is utterance two of the Voice AI websocket relay test.",
    "This is utterance three of the Voice AI websocket relay test.",
]


logger = logging.getLogger("tts_telnyx_test_baseline")
...
def _validate_config() -> None:
    ...
    if (TTS_AUDIO_FORMAT, TELNYX_AUDIO_FORMAT) not in SUPPORTED_CODEC_PATHS:
        raise RuntimeError(
            "Unsupported codec path for this baseline: "
...
def _to_media_frames(payload_b64: str) -> list[str]:
    raw = _convert_upstream_raw_for_telnyx(base64.b64decode(payload_b64))
    if not raw:
        return []

    frames: list[str] = []
    for start in range(0, len(raw), TELNYX_FRAME_BYTES):
        frame = raw[start : start + TELNYX_FRAME_BYTES]
        if len(frame) < TELNYX_FRAME_BYTES:
            frame = frame + (SILENCE_BYTE * (TELNYX_FRAME_BYTES - len(frame)))
...
async def _send_silence_frames(websocket: WebSocket, duration_ms: float) -> None:
    frame_count = max(1, int(round(duration_ms / (FRAME_DURATION_S * 1000.0))))
    stats = RelayStats()
...
async def _reader(tts_ws, context_id: str, queue: asyncio.Queue[str], done_event: asyncio.Event, stats: RelayStats) -> None:
    while True:
        ...
        if audio_payload:
            ...
            for frame_payload in _to_media_frames(audio_payload):
                if queue.full():
...
async def _player(websocket: WebSocket, queue: asyncio.Queue[str], done_event: asyncio.Event, stats: RelayStats) -> None:
    prebuffer_frames = max(0, int(round(PLAYOUT_PREBUFFER_MS / (FRAME_DURATION_S * 1000.0))))
    deadline = time.monotonic()

    while queue.qsize() < prebuffer_frames and not done_event.is_set():
...
async def relay_tts_utterance(websocket: WebSocket, text: str, utterance_index: int) -> None:
    ...
    logger.info(
        "Starting utterance %s/%s delivery_mode=%s playout_prebuffer_ms=%.0f tts_audio_format=%s telnyx_audio_format=%s",
        utterance_index,
        len(TTS_TEXTS),
        TTS_API_PARAMS["delivery_mode"],
        PLAYOUT_PREBUFFER_MS,
        TTS_AUDIO_FORMAT,
        TELNYX_AUDIO_FORMAT,
    )
    ...
    logger.info(
        "Completed utterance %s/%s frames_sent=%s underruns=%s late_frames=%s max_late_ms=%.1f upstream_messages=%s",
        utterance_index,
        len(TTS_TEXTS),
        stats.frames_sent,
        stats.underruns,
